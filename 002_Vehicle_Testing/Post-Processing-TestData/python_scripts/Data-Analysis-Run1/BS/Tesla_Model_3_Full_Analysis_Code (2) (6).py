...
# Define a function to create the plots in a 10x1 grid
def create_insightful_plots_10x1(data):
    fig, axs = plt.subplots(10, 1, figsize=(10, 35), sharex=True)
    fig.suptitle('Insightful Visualizations of Tesla Model 3 Data', fontsize=16)
    
    # Plot 1: Vehicle Speed and Brake Torques Over Time
    ax = axs[0]
    ax.plot(data.index, data['DI_vehicleSpeed'], label='Vehicle Speed (KPH)', color='black')
    ax.plot(data.index, data['ESP_brakeTorqueFrL'], label='Front Left Brake Torque (Nm)', color='blue')
    ax.plot(data.index, data['ESP_brakeTorqueFrR'], label='Front Right Brake Torque (Nm)', color='green')
    ax.plot(data.index, data['ESP_brakeTorqueReL'], label='Rear Left Brake Torque (Nm)', color='red')
    ax.plot(data.index, data['ESP_brakeTorqueReR'], label='Rear Right Brake Torque (Nm)', color='purple')
    ax.set_title('Vehicle Speed and Brake Torques Over Time')
    ax.legend()
    ax.grid(True)
    
    # Plot 2: Wheel Speeds Over Time
    ax = axs[1]
    ax.plot(data.index, data['WheelSpeedFL175'], label='Front Left Wheel Speed (KPH)', color='blue')
    ax.plot(data.index, data['WheelSpeedFR175'], label='Front Right Wheel Speed (KPH)', color='green')
    ax.plot(data.index, data['WheelSpeedRL175'], label='Rear Left Wheel Speed (KPH)', color='red')
    ax.plot(data.index, data['WheelSpeedRR175'], label='Rear Right Wheel Speed (KPH)', color='purple')
    ax.set_title('Wheel Speeds Over Time')
    ax.legend()
    ax.grid(True)
    
    # Plot 3: Longitudinal Acceleration and Brake Torques
    ax = axs[2]
    ax.plot(data.index, data['RCM_longitudinalAccel'], label='Longitudinal Acceleration (m/s²)', color='green')
    ax.plot(data.index, data['ESP_brakeTorqueFrL'], label='Front Left Brake Torque (Nm)', color='blue')
    ax.plot(data.index, data['ESP_brakeTorqueFrR'], label='Front Right Brake Torque (Nm)', color='green')
    ax.set_title('Longitudinal Acceleration and Brake Torques')
    ax.legend()
    ax.grid(True)
    
    # Plot 4: Lateral Acceleration and Yaw Rate
    ax = axs[3]
    ax.plot(data.index, data['RCM_lateralAccel'], label='Lateral Acceleration (m/s²)', color='blue')
    ax.plot(data.index, data['RCM_yawRate'], label='Yaw Rate (rad/s)', color='purple')
    ax.set_title('Lateral Acceleration and Yaw Rate')
    ax.legend()
    ax.grid(True)
    
    # Plot 5: Vertical Acceleration and Roll Rate
    ax = axs[4]
    ax.plot(data.index, data['RCM_verticalAccel'], label='Vertical Acceleration (m/s²)', color='red')
    ax.plot(data.index, data['RCM_rollRate'], label='Roll Rate (rad/s)', color='blue')
    ax.set_title('Vertical Acceleration and Roll Rate')
    ax.legend()
    ax.grid(True)
    
    # Plot 6: Brake Torques Distribution
    ax = axs[5]
    brake_torques = [data['ESP_brakeTorqueFrL'].mean(), data['ESP_brakeTorqueFrR'].mean(), data['ESP_brakeTorqueReL'].mean(), data['ESP_brakeTorqueReR'].mean()]
    ax.bar(['Front Left', 'Front Right', 'Rear Left', 'Rear Right'], brake_torques, color=['blue', 'green', 'red', 'purple'])
    ax.set_title('Average Brake Torque Distribution')
    ax.set_ylabel('Brake Torque (Nm)')
    
    # Plot 7: Correlation Heatmap
    ax = axs[6]
    correlation_matrix = data[['DI_vehicleSpeed', 'ESP_brakeTorqueFrL', 'ESP_brakeTorqueFrR', 'ESP_brakeTorqueReL', 'ESP_brakeTorqueReR', 
                               'WheelSpeedFL175', 'WheelSpeedFR175', 'WheelSpeedRL175', 'WheelSpeedRR175', 
                               'RCM_lateralAccel', 'RCM_longitudinalAccel', 'RCM_verticalAccel', 
                               'RCM_yawRate', 'RCM_pitchRate', 'RCM_rollRate']].corr()
    sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', ax=ax)
    ax.set_title('Correlation Matrix of Key Variables')
    
    # Plot 8: Pitch Rate and Vertical Acceleration
    ax = axs[7]
    ax.plot(data.index, data['RCM_pitchRate'], label='Pitch Rate (rad/s)', color='green')
    ax.plot(data.index, data['RCM_verticalAccel'], label='Vertical Acceleration (m/s²)', color='red')
    ax.set_title('Pitch Rate and Vertical Acceleration')
    ax.legend()
    ax.grid(True)
    
    # Plot 9: Longitudinal Acceleration and Vehicle Speed
    ax = axs[8]
    ax.plot(data.index, data['RCM_longitudinalAccel'], label='Longitudinal Acceleration (m/s²)', color='green')
    ax.plot(data.index, data['DI_vehicleSpeed'], label='Vehicle Speed (KPH)', color='black')
    ax.set_title('Longitudinal Acceleration and Vehicle Speed')
    ax.legend()
    ax.grid(True)
    
    # Plot 10: Lateral Acceleration and Vehicle Speed
    ax = axs[9]
    ax.plot(data.index, data['RCM_lateralAccel'], label='Lateral Acceleration (m/s²)', color='blue')
    ax.plot(data.index, data['DI_vehicleSpeed'], label='Vehicle Speed (KPH)', color='black')
    ax.set_title('Lateral Acceleration and Vehicle Speed')
    ax.legend()
    ax.grid(True)
...
# Create the insightful plots in a 10x1 grid
create_insightful_plots_10x1(stability_data)

...
# Compare brake torque and yaw rate by plotting them together

# Define the metrics to compare
metrics_to_compare = ['ESP_brakeTorqueFrL', 'ESP_brakeTorqueFrR', 'ESP_brakeTorqueReL', 'ESP_brakeTorqueReR', 'RCM_yawRate']

# Plot brake torques and yaw rate
fig, ax1 = plt.subplots(figsize=(15, 8))

# Plot brake torques on the left y-axis
color = 'tab:blue'
ax1.set_xlabel('Time Index')
ax1.set_ylabel('Brake Torque (Nm)', color=color)
ax1.plot(stability_data.index, stability_data['ESP_brakeTorqueFrL'], label='Front Left Brake Torque', color='blue', alpha=0.5)
ax1.plot(stability_data.index, stability_data['ESP_brakeTorqueFrR'], label='Front Right Brake Torque', color='green', alpha=0.5)
ax1.plot(stability_data.index, stability_data['ESP_brakeTorqueReL'], label='Rear Left Brake Torque', color='red', alpha=0.5)
ax1.plot(stability_data.index, stability_data['ESP_brakeTorqueReR'], label='Rear Right Brake Torque', color='purple', alpha=0.5)
ax1.tick_params(axis='y', labelcolor=color)
ax1.legend(loc='upper left')
ax1.grid(True)

# Plot yaw rate on the right y-axis
ax2 = ax1.twinx()
color = 'tab:orange'
ax2.set_ylabel('Yaw Rate (rad/s)', color=color)
ax2.plot(stability_data.index, stability_data['RCM_yawRate'], label='Yaw Rate', color=color, alpha=0.7)
ax2.tick_params(axis='y', labelcolor=color)
ax2.legend(loc='upper right')

fig.tight_layout()
plt.title('Comparison of Brake Torques and Yaw Rate')
plt.show()

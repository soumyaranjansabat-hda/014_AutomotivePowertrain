...
# Calculate energy recovery during braking
time_diffs = np.diff(data_cleaned['Time (relative)'].to_numpy())
front_energy_recovered = np.sum(data_cleaned['FrontPower2E5'].iloc[:-1] * time_diffs)
rear_energy_recovered = np.sum(data_cleaned['RearPower266'].iloc[:-1] * time_diffs)

# Analyze SOC changes during braking
soc_columns = ['SOCave292', 'SOCmax292', 'SOCmin292', 'SOCUI292']
available_soc_columns = [col for col in soc_columns if col in data_cleaned.columns]
soc_data = data_cleaned[['Time (relative)'] + available_soc_columns]
soc_data[available_soc_columns] = soc_data[available_soc_columns].apply(pd.to_numeric, errors='coerce')

# Correlate braking with vehicle speed and motor power outputs
if 'DI_vehicleSpeed' in data_cleaned.columns:
    speed_and_power_data = data_cleaned[['Time (relative)', 'DI_vehicleSpeed', 'FrontPower2E5', 'RearPower266']]
    speed_and_power_data['DI_vehicleSpeed'] = pd.to_numeric(speed_and_power_data['DI_vehicleSpeed'], errors='coerce')

    # Plotting vehicle speed and motor power outputs during braking
    fig, ax1 = plt.subplots(figsize=(14, 7))
    color = 'tab:red'
    ax1.set_xlabel('Time (relative in sec)')
    ax1.set_ylabel('Speed (km/h)', color=color)
    ax1.plot(speed_and_power_data['Time (relative)'], speed_and_power_data['DI_vehicleSpeed'], color=color)
    ax1.tick_params(axis='y', labelcolor=color)

    ax2 = ax1.twinx()
    color = 'tab:blue'
    ax2.set_ylabel('Power (kW)', color=color)
    ax2.plot(speed_and_power_data['Time (relative)'], speed_and_power_data['FrontPower2E5'], label='Front Motor Power (kW)', color='blue')
    ax2.plot(speed_and_power_data['Time (relative)'], speed_and_power_data['RearPower266'], label='Rear Motor Power (kW)', color='green')
    ax2.tick_params(axis='y', labelcolor=color)
    ax2.legend(loc='upper right')
    plt.title('Vehicle Speed and Motor Power Outputs During Braking Events')
    plt.show()

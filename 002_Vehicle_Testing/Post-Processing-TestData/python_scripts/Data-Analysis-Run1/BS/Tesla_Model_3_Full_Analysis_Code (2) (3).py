...
# Data preparation and analysis for motor efficiency
data['Operation_Mode_Front'] = data['FrontPower2E5'].apply(lambda x: 'Regeneration' if x < 0 else 'Acceleration')
data['Operation_Mode_Rear'] = data['RearPower266'].apply(lambda x: 'Regeneration' if x < 0 else 'Acceleration')

# Enhanced plots with color coding for better visualization of operational modes
fig, axs = plt.subplots(2, 1, figsize=(12, 12))
sns.scatterplot(x='DI_vehicleSpeed', y='FrontPower2E5', data=data, hue='Operation_Mode_Front', style='Operation_Mode_Front', ax=axs[0], palette={'Acceleration': 'blue', 'Regeneration': 'red'})
axs[0].set_title('Front Motor Power Output vs Vehicle Speed (Colored by Operation Mode)')
axs[0].set_ylabel('Motor Power (kW)')
axs[0].grid(True)

sns.scatterplot(x='DI_vehicleSpeed', y='RearPower266', data=data, hue='Operation_Mode_Rear', style='Operation_Mode_Rear', ax=axs[1], palette={'Acceleration': 'green', 'Regeneration': 'orange'})
axs[1].set_title('Rear Motor Power Output vs Vehicle Speed (Colored by Operation Mode)')
axs[1].set_xlabel('Vehicle Speed (kph)')
axs[1].set_ylabel('Motor Power (kW)')
axs[1].grid(True)
